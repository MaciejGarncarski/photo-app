...
export const mapChatUsers = ({
	...
	const hasSentLastMessage =
		...
		sentMessage.createdAt < receivedMessage.createdAt
	const message = hasSentLastMessage
		? `You: ${receivedMessage.text}`
		: sentMessage?.text

	return {
		userId,
		message: message,
		messageCreatedAt: (hasSentLastMessage
			? receivedMessage?.createdAt
			: sentMessage?.createdAt
		)?.toString(),
	}
